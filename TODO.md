...
### [x] T-030 首个可信公开 Release、本机安装与安装后启动
...
| R-028 | T-028 | 新增可信自动更新、完整数据/二进制备份、自动恢复、显式回滚与安全卸载 | `internal/update/`、`internal/db/` 跨进程维护锁、CLI update/rollback/uninstall 与 Windows helper、安装/卸载脚本接线、README/部署文档、`TODO.md` | 活动 Store 未关闭时不复制 SQLite；签名/hash/size/磁盘/备份/锁未通过不切换；备份不原地修改且逐文件复验；切换后失败恢复旧二进制和完整匹配数据目录；默认卸载保留数据/凭据，purge 绑定收据并要求精确二次确认，拒绝宽泛/符号链接/安装重叠目录；Lite 与 Runner 隔离边界不变；`.interviewcraft/` 保持忽略且未暂存 | 严格 Release 客户端验证固定 tag workflow identity/OIDC、六平台 manifest、归档 hash/size/路径与内嵌版本；`update --check` 无更新成功且不建备份，八阶段 Pending/Streaming/终态可观测；token 维护锁阻止活动 SQLite 和并发更新，备份保留空目录/模式并拒绝链接/特殊文件/篡改；Windows helper 调度/Finalize、Linux 原生 `flock`/rename、迁移/doctor/替换/取消/断电 PhaseSwitched、低磁盘、签名/checksum、备份损坏和回滚目标自检失败均故障注入，自动恢复与 forward snapshot 通过；默认卸载保留数据库/凭据，purge 收据绑定、二次确认、凭据库失败闭合和宽目录拒绝通过；根模块无缓存全测 56.9s 退出 0，update/db/cli 覆盖率 64.2%/72.4%/47.9%；Docker-free 完整质量门 461.2s 退出 0，含两模块 verify/vet/test、迁移、fresh-install、发布/Runner/安装器 fixture 和六目标构建；Windows+Linux 安装器夹具 168.8s 退出 0，Linux update 原生测试 1.875s；真实 Runner 隔离门 65.2s 退出 0且零残留；macOS 更新测试二进制已交叉编译但无本地真机 runner，明确为本地边界未运行 | `feat(update): T-028 add verified upgrade and rollback` |
| R-029 | T-029 | 固化 Lite、Private Local、Full Practice 的一键安装到保留数据卸载全链路，并生成可审计平台证据 | 部署缺陷所需的 `internal/update/`/CLI 最小修复、`internal/e2e/`、`scripts/` 部署 fixture/门禁、`.github/workflows/`、README、部署/安全/质量/贡献文档、`TODO.md` | 未降低 T-001～T-028 的隐私、证据、签名、备份或隔离门禁；Lite/Private Local 继续不接触 Docker/Registry且不依赖外部数据库/语言运行时/管理员权限；无效签名和任何切换失败均保留原版本与完整数据；`.interviewcraft/`、`dist/`、证据 JSON、临时工具链与 PowerShell 缓存均未暂存 | 新增真实进程 E2E：Windows PowerShell 5.1 在隔离 HOME/PATH 上让 Lite 与 Private Local 分别完成 `安装 → 同版本幂等重装 → setup → doctor → 80×24 run → 重启 → update --check → 错签拒绝且数据/版本不变 → 安装器委托 1.1.0 升级 → doctor → 无更新空态 → rollback → uninstall`，正式证据门 125.018s 退出 0；完整 Lite 训练覆盖画像、场景、文字面试、Coach、代码 seam、15/15 证据报告、下一轮、迁移与恢复运行；Linux `golang:1.26` 只读容器以原生 POSIX 安装/rename/flock 路径 48.9s 退出 0（生命周期 17.19s）；Full Practice 汇总 167.681s 退出 0，Full setup 签名/取消/恢复、Runner manifest、Python/JavaScript/Java、超时/OOM/禁网/进程攻击及零残留均通过；四态矩阵把安装/setup、训练、update/rollback、Runner、uninstall 映射到可编译测试名，覆盖无 Provider/历史/Runner/更新/备份、Release/网络/签名/hash、凭据库、目录/SQLite/终端、Registry/Runner、迁移/回滚和可重试恢复；E2E 实测发现并修复 Windows 正在使用 EXE 的短暂 Access Denied/Sharing/Lock 占用，以 15s 有界重试和本地诊断处理，失败仍整体回滚；Docker-free 质量门 158.3s、完整质量门 213.8s 均退出 0，含 gofmt/diff、部署契约、两模块 verify/vet/覆盖测试、迁移、fresh-install、发布/Runner/安装器 fixture、六目标构建与真实隔离；actionlint/ShellCheck 全工作流零告警；CI 使用 README 原命令覆盖 `windows-latest`/`ubuntu-latest`/`macos-latest` 并上传含平台、架构、1.0.0/1.1.0、Go 版本、提交 SHA、dirty 标记、UTC 与耗时的证据；本地实跑仅 Windows/Linux/amd64，macOS 真机、arm64、GitHub OIDC/GHCR/Release 网络路径必须由未推送的 CI/正式发布执行，未伪报通过 | `chore(deployment): T-029 certify one-click full deployment` |
| R-030 | T-030 | 发布首个通过完整供应链门禁的 v1.0.1 公开 Release，修复 Windows PowerShell 7 日期解析测试，并完成正式签名资产的安装后启动验证 | `scripts/test-release-metadata.ps1`、`TODO.md`、Git tag v1.0.1、GitHub Release/GHCR 元数据 | 不移动/覆盖已有 tag；不降低签名/OIDC/manifest/hash/size/Draft 回读/Runner 隔离门禁；Lite 继续无 Docker；不提交证据 JSON/密钥/缓存 | 本地 Go 1.26.7 下 `go mod verify`、根模块与 Runner agent `go test -count=1 ./...`、`test-release-quality.ps1 -SkipRunnerIsolation` 全部退出 0；Windows 部署 E2E Lite/Private Local 395.446s 退出 0（worktree dirty=false）；PowerShell 5.1/7 均通过 runner-release/release-metadata（修复 ConvertFrom-Json 将 build_time 转为 DateTime 导致的 PS7 比较失败）与 PS7 installer fixture；推送注解 tag v1.0.1 触发 release workflow run 32585537366 成功，六平台归档、双架构 Runner、6 SBOM、provenance、严格清单与 Sigstore bundle 回读后 `draft=false` 公开；本机以真实 v1.0.1 23 个资产和真实 Cosign v3.1.3 验证 manifest，安装器完成 7 阶段 `setup → doctor`，新进程 PATH 发现 `interviewcraft`，`version`=1.0.1/commit=777d4b6，`doctor` 退出 0，`run --once` 120×36 退出 0；沙箱 PowerShell TLS 代理不可用，故下载走本地回环镜像，签名/身份/哈希/内嵌版本验证均使用正式 Release 资产 | `chore(release): T-030 publish and install the first verified release` |
